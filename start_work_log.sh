...
function add_work()
{
    ...
    done < "$PENDING_WORK_FILE"

    if [ "$found_match" = false ]; then
        echo "$work_entry" >> "$PENDING_WORK_FILE"
    fi
}
...
taskchoice=$(cut -d'"' -f 2 $PENDING_WORK_FILE | sed '{x;p;x;}' | \
...
# 选择其他时
[[ $taskchoice = *"其他"* ]] && {
    taskother=$(zenity --title="补充说明" --text "其他工作" --entry --timeout 600 --width 600 --height 100)
...
# 选择除了其他外的选项
[ x"$taskchoice" != x"其他" ] && {
    [  x"$taskchoice" != x"" ] && echo "[$timenow]: $taskchoice" >> "$WORK_LOG_FILE"
}
